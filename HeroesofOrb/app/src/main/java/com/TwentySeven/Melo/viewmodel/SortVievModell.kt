package com.TwentySeven.Melo.viewmodel
...
import com.TwentySeven.Melo.api.CountryCodeJS
import com.TwentySeven.Melo.api.GeoDev
import com.TwentySeven.Melo.api.ServiceApi
...
import com.TwentySeven.Melo.nev.DataFromApiResource
...
@HiltViewModel
class SortVievModell @Inject constructor(private val application: Application) : ViewModel() {
    ...
    private fun saveSharedPref(key: String, data: String?) {
        val sharedPreferences = application.getSharedPreferences(Constance.KEY_MAIN_FOR_SHARED_PREF, Constance.MODE_PRIVATE)
    ...
    fun iniSettingVievModel() {
        ...
        _appLinkDataaaa.value = getFromSharedPref(Constance.KEY_SHARED_PREF_APPLINK_DATA, null)
        _appsFlyerDattaGotten.value = getFromSharedPref(Constance.KEY_SHARED_PREF_APPS_FLY_DATA, null)
        _link.value = getFromSharedPref(Constance.KEY_SHARED_PREF_LINK, Constance.KEY_NOOOOO_DATA)
        _advertisingIdClient.value = getFromSharedPref(Constance.KEY_SHARED_PREF_ADVERT_ID, Constance.KEY_NOOOOO_DATA)
    ...
    fun fetchDeferredAppLinkData(context: Context) {
        AppLinkData.fetchDeferredAppLinkData(context) { appLinkData: AppLinkData? ->
            appLinkData?.let {
                saveSharedPref(Constance.KEY_SHARED_PREF_APPLINK_DATA, it.targetUri.host.toString())
            ...
            "1" -> {
                ...
                while (true) {
                    if (naming != null && deep != null) {
                        ...
                        if (deep!!.contains(Constance.KEY_TDB2) || naming!!.contains(Constance.KEY_TDB2) || listOfAllGeo.contains(userGeo)
                        ) {
                        ...
                        delay(1000)
                        if (naming == null) {
                            naming = getFromSharedPref(Constance.KEY_SHARED_PREF_APPS_FLY_DATA, null)
                        }
                        if (deep == null) {
                            deep = getFromSharedPref(Constance.KEY_SHARED_PREF_APPLINK_DATA, null)
                        }
                    }
                }
            }
            ...
            "55" -> {
                ...
                if (listOfAllGeo.contains(userGeo)) {
                    CoroutineScope(Dispatchers.IO).launch {
                        while (true) {
                            if (appLinkDataaaa.value != null) {
                                val dataAppLinkData = appLinkDataaaa.value ?: Constance.KEY_NOOOOO_DATA
                                if (dataAppLinkData.contains(Constance.KEY_TDB2) || listOfAllGeo.contains(userGeo)) {
    ...
    fun initAppsFlyerLib(context: Context) {
        AppsFlyerLib.getInstance().init("zhfAbWaoJkfNLGXyiUsWSB", conversionDataListener, application)
